...
// PARTIR un pedido en N viajes (entrega parcial por capacidad de camión).
// body.partes = [{kg,porte,lineas:[{id,cantidad}]}, ...]. partes[0] se QUEDA en el
// pedido original (conserva su carga); partes[1..] son pedidos nuevos en libres.
// Cada línea reparte su cantidad entre los viajes; kg/porte se asignan por viaje
// (clave: que los KPIs no dupliquen). Todo en UNA transacción → seguro offline.
router.post('/api/pedidos/:id/partir', async (req, res) => {
  const partes = Array.isArray(req.body && req.body.partes) ? req.body.partes : [];
  if (partes.length < 2) return res.status(400).json({error:'Hacen falta al menos 2 viajes'});
  const cantDe = (parte, lineId) => {
    const l = (parte.lineas||[]).find(x => String(x.id) === String(lineId));
    return l ? Math.max(0, Number(l.cantidad)||0) : 0;
  };
    ...
    for (let i = 1; i < partes.length; i++) {
      ...
      for (const ln of lineas) {
        ...
        await client.query(
          ...
          [p2.id, ln.referencia, ln.descripcion, q, ln.observaciones, ln.orden, ln.embalaje, upp!=null?Math.round(upp*q*100)/100:null]);
      }
      nuevos.push(p2);
    }
    // partes[0] → se queda en el pedido original; ajustar cantidades de cada línea
    const p0 = partes[0];
    for (const ln of lineas) {
      const q = cantDe(p0, ln.id);
      if (q <= 0) { await client.query('DELETE FROM pedido_lineas WHERE id=$1',[ln.id]); continue; }
      const upp = kgsPorUd(ln);
